...
from Shadow.utils.database import get_served_chats, get_served_users
...
async def broadcast(client: Client, message: Message):
    ...
    if not is_pro_user:
        return await message.reply_text(
            f"{user.mention}, ðšˆðš˜ðšž ðšðš˜ðš—'ðš ðš‘ðšŠðšŸðšŽ ðšðš˜ ðšŠðšŒðšŒðšŽðšœðšœ. ðšŒðš˜ðš—ðš—ðšŽðšŒðš [á´…Ê™á´¢á´›á´‡á´€á´](https://t.me/DBZ_COMMUNITY2) ðšŠðš—ðš ðš‹ðšžðš¢ ðš’ðš.."
        )
    if IS_BROADCASTING:
        return await message.reply_text(
    ...
    for pro_user in pro_users:
        ...
        try:
            ...
            pro_list_text += (
                ...
                f"Expires â€¢ <code>{expires}</code>\n"
                f"Added on â€¢ <code>{added_time}</code>\n"
                f"Added by â€¢ {added_by_user.mention}\n\n"
            )
        except Exception:
            pro_list_text += (
                f"User ID â€¢ {user_id}\n"
                f"Expires â€¢ {expires}\n"
                f"Added on â€¢ {added_time}\n"
                f"Added by â€¢ {added_by}\n\n"
            )
